...
@legacy_execution_router.api_route(
    "/{shift_id}/rooms/{room_id}/tasks/{task_id}/toggle",
    methods=["POST", "PATCH"],
    response_model=ShiftExecutionStateResponse,
    summary="Toggle Room Task Completion State (Deprecated)",
    description="Deprecated: Task completion is photo-driven upon manager review.",
...
async def toggle_room_task_completion(
    ...
    has_submitted_photos = len(target_room.get("submitted_photos", [])) > 0
    if len(tasks) > 0 and all(t.get("is_completed") for t in tasks):
        if has_submitted_photos:
...
async def upload_worker_room_photo(
    ...
    clean_photo_name = None
    if shift_doc:
        for r in shift_doc.get("rooms", []):
            if str(r.get("id") or r.get("room_id") or r.get("name")) == str(room_id):
                for t in r.get("tasks", []):
                    if task_id and str(t.get("id")) == str(task_id):
                        clean_photo_name = t.get("name") or t.get("photo_name")
                    for p in t.get("photo", []):
                        if (photo_id and str(p.get("id")) == str(photo_id)) or p.get("name"):
                            clean_photo_name = p.get("name") or clean_photo_name
                for rp in r.get("required_photos", []):
                    if (photo_id and str(rp.get("id")) == str(photo_id)) or (photo_type and rp.get("photo_type") == photo_type):
                        clean_photo_name = rp.get("name") or clean_photo_name
    if not clean_photo_name:
        clean_photo_name = f"{r_name} {photo_type.capitalize()} Photo"
    ...
    if shift_doc:
        rooms = shift_doc.get("rooms", [])
        target_room = next((r for r in rooms if str(r.get("id") or r.get("room_id") or r.get("name")) == room_id), None)
        if target_room:
            ...
            tasks = target_room.get("tasks", [])
            all_done = len(tasks) > 0 and all(t.get("is_completed") for t in tasks)
            if all_done:
                target_room["status"] = "photo_submitted"
